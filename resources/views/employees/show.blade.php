    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-bold text-2xl text-gray-800 leading-tight">
                Detail Mitra: {{ $employee->full_name }}
            </h2>
            <div class="flex gap-2">
                <a href="{{ route('partners.edit', $employee) }}" class="inline-flex items-center px-4 py-2 bg-amber-500 hover:bg-amber-600 border border-transparent rounded-xl font-semibold text-xs text-white uppercase tracking-widest transition duration-150 shadow-md shadow-amber-100">
                    Edit Profil
                </a>
            ...
            <div class="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl shadow-sm p-6 text-white flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <div class="flex items-center gap-3">
                        <span class="text-3xl font-extrabold tracking-tight">{{ $employee->full_name }}</span>
                        <span class="bg-white/20 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
                            {{ $employee->mitra_id }}
                        </span>
                    </div>
                    <p class="text-sm text-blue-100 mt-1">Bergabung sejak {{ $employee->joined_date?->translatedFormat('d F Y') }} ({{ $employee->joined_date?->diffForHumans() }})</p>
                </div>
                <div class="flex gap-4">
                    <div class="bg-white/10 backdrop-blur-md px-4 py-3 rounded-xl">
                        ...
                        <span class="block text-lg font-bold uppercase mt-0.5">{{ $employee->status }}</span>
                    </div>
                    <div class="bg-white/10 backdrop-blur-md px-4 py-3 rounded-xl">
                        <span class="block text-xs text-blue-100 uppercase font-semibold">Rate per Jam</span>
                        <span class="block text-lg font-bold mt-0.5">Rp{{ number_format($employee->finance->base_hourly_rate ?? 54000, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
                    ...
                    <div>
                        ...
                        <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6 mt-4">
                            <div>
                                ...
                                <dd class="text-sm font-medium text-gray-900 mt-1">{{ $employee->nik }}</dd>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <dd class="text-sm font-medium text-gray-900 mt-1">
                                    {{ $employee->birth_place_date }}
                                </dd>
                            </div>
                            <div>
                                <dt class="text-xs font-semibold text-gray-400 uppercase">Nomor WhatsApp</dt>
                                <dd class="text-sm font-medium text-gray-900 mt-1">
                                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $employee->whatsapp_number) }}" target="_blank" class="text-indigo-600 hover:underline flex items-center gap-1">
                            ...
                            <div>
                                ...
                                <dd class="text-sm font-medium text-gray-900 mt-1">{{ $employee->email }}</dd>
                            </div>
                        ...
                        <div class="overflow-x-auto mt-4">
                            <table class="min-w-full divide-y divide-gray-100">
                                ...
                                <tbody class="divide-y divide-gray-100 bg-white">
                                    @forelse($employee->videoSubmissions as $submission)
                                        <tr>
                                            <td class="px-4 py-3 whitespace-nowrap text-sm font-semibold text-gray-900">{{ $submission->app_video_id }}</td>
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{{ $submission->task_category }}</td>
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{{ $submission->submitted_duration_formatted }}</td>
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 font-medium">{{ $submission->approved_duration_formatted }}</td>
                                            <td class="px-4 py-3 whitespace-nowrap">
                                                @php
                                                    $statusClasses = [
                                                        ...
                                                        'retake' => 'bg-amber-100 text-amber-800',
                                                    ];
                                                @endphp
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold {{ $statusClasses[$submission->qc_status] ?? 'bg-gray-100' }}">
                                                    {{ ucfirst($submission->qc_status) }}
                                                </span>
                                            </td>
                                        </tr>
                                    @empty
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-100 p-6 space-y-4">
                        ...
                        <dl class="space-y-4">
                            <div>
                                ...
                                <dd class="text-sm font-medium text-gray-900">{{ $employee->equipment->smartphone_type ?? '-' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-semibold text-gray-400 uppercase">Status Headstrap</dt>
                                <dd class="mt-1">
                                    @php
                                        $headstrapStatus = $employee->equipment->headstrap_status ?? 'not_owned';
                                        $headstrapClasses = [
                                            'not_owned' => 'bg-gray-100 text-gray-800',
                                            'reimburse_process' => 'bg-yellow-100 text-yellow-800',
                            ...
                            @if($employee->equipment && $employee->equipment->nominal_reimburse > 0)
                                <div>
                                    <dt class="text-xs font-semibold text-gray-400 uppercase">Nominal Reimburse</dt>
                                    <dd class="text-sm font-bold text-gray-900">Rp{{ number_format($employee->equipment->nominal_reimburse, 0, ',', '.') }}</dd>
                                </div>
                            @endif
                            @if($employee->equipment && $employee->equipment->headstrap_receipt_path)
                                <div>
                                    ...
                                    <dd class="text-sm font-medium text-indigo-600 hover:underline mt-1">
                                        <a href="{{ asset('storage/' . $employee->equipment->headstrap_receipt_path) }}" target="_blank" class="flex items-center gap-1">
                                            Unduh Nota
                                            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-100 p-6 space-y-4">
                        ...
                        <dl class="space-y-4">
                            <div>
                                ...
                                <dd class="text-sm font-medium text-gray-900">{{ $employee->finance->bank_name ?? '-' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-semibold text-gray-400 uppercase">Nomor Rekening</dt>
                                <dd class="text-sm font-bold text-gray-900 tracking-wider">{{ $employee->finance->account_number ?? '-' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-semibold text-gray-400 uppercase">Atas Nama Pemilik</dt>
                                <dd class="text-sm font-medium text-gray-900">{{ $employee->finance->account_owner_name ?? '-' }}</dd>
                            </div>
                        </dl>
                    </div>
