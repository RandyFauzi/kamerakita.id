<x-app-layout>
    <div class="space-y-6" x-data="{ showBanner: true }">
        ...
        <template x-if="showBanner">
            <div class="bg-white rounded-3xl p-6 border border-gray-150 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative animate-in fade-in slide-in-from-top-4 duration-300">
                <div class="flex gap-4 items-start">
                    ...
                    <div class="space-y-1">
                        <span class="block text-xs font-black tracking-widest text-blue-600 uppercase">AKUN MITRA AKTIF</span>
                        <p class="text-sm text-gray-500 max-w-xl">Lengkapi otentikasi akun Anda dan unggah minimal 1 laporan kerja video harian untuk mengamankan pendapatan bonus.</p>
                        <!-- Progress bar -->
                ...
                <div class="flex items-center gap-3">
                    <a href="{{ route('video-submissions.submit-report.create') }}" class="px-5 py-2.5 bg-white border border-gray-200 hover:bg-gray-50 rounded-2xl text-xs font-bold text-gray-700 shadow-sm transition">
                        Kirim Laporan Baru
                    </a>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 bg-gradient-to-r from-sky-200 via-pink-100 to-yellow-100 rounded-[32px] p-8 border border-white/40 shadow-sm flex flex-col justify-between min-h-[220px] relative overflow-hidden group">
                ...
                <div class="relative z-10">
                    <span class="block text-xs font-bold uppercase tracking-wider text-slate-500 font-mono">ESTIMASI PENDAPATAN ANDA (RATE Rp{{ number_format($metrics['hourly_rate'], 0, ',', '.') }}/JAM)</span>
