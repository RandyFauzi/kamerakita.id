<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div class="flex gap-2">
                ...
                <form action="{{ route('payroll.mark-as-paid') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menandai semua tagihan approved sebagai paid?')">
                    @csrf
                    <button type="submit" class="inline-flex items-center px-4 py-2.5 bg-emerald-500 hover:bg-emerald-600 border border-transparent rounded-xl font-semibold text-xs text-white uppercase tracking-widest transition shadow-md shadow-emerald-100">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="py-4 space-y-6" x-data="{ showBanner: true }">
        ...
        <template x-if="showBanner">
            <div class="bg-white rounded-3xl p-6 border border-gray-150 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative animate-in fade-in slide-in-from-top-4 duration-300">
                ...
                <div class="flex items-center gap-3">
                    <a href="{{ route('video-submissions.qc-room') }}" class="px-5 py-2.5 bg-indigo-600 text-white rounded-2xl text-xs font-bold shadow-sm hover:bg-indigo-700 transition">
                        Buka QC Room
                    </a>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 bg-gradient-to-r from-purple-200 via-sky-100 to-yellow-100 rounded-[32px] p-8 border border-white/40 shadow-sm flex flex-col justify-between min-h-[220px] relative overflow-hidden group">
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex items-baseline gap-2 mt-3">
                        <span class="text-4xl font-black text-slate-900 tracking-tight">{{ $metrics['global_all_time_hours_formatted'] }}</span>
