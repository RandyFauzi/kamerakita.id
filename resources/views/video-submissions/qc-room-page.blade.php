<x-app-layout>
    ...
    <div class="py-8" x-data="{ 
                                            ...
                                            <div class="flex flex-col">
                                                ...
                                                <span class="text-[10px] text-slate-400 font-mono">ID: {{ $partner->mitra_id }}</span>
                                                 <div class="flex items-center gap-1.5 mt-0.5 text-xs text-slate-400" x-data="{ copied: false }" @click.stop>
                                                     <span class="select-all">{{ $partner->email ?: 'Email App Belum Diset' }}</span>
                                                     @if($partner->email)
                                                         <button type="button" 
                                                                 @click="navigator.clipboard.writeText('{{ $partner->email }}'); copied = true; setTimeout(() => copied = false, 1500)"
                                                                 class="text-gray-400 hover:text-indigo-650 transition p-0.5 rounded"
                                                                 title="Salin Email Aplikasi">
        ...
        <div x-show="showCreateModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" x-cloak style="display: none;">
            <div class="bg-white rounded-[32px] max-w-3xl w-full shadow-2xl overflow-hidden border border-gray-150 animate-in fade-in zoom-in-95 duration-200 flex flex-col my-8 max-h-[90vh]">
                <!-- Modal Header -->
                        ...
                        <div x-data="{
                            ...
                            workers: [
                                @foreach($allWorkers as $worker)
                                    { id: '{{ $worker->id }}', name: '{{ addslashes($worker->full_name) }} ({{ addslashes($worker->email ?: 'Email App Belum Diset') }})' },
                                @endforeach
                            ],
                            get filteredWorkers() {
